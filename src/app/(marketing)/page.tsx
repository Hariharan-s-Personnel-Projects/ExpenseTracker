...
import { ArrowRight, Bot, PieChart, ShieldCheck, Sparkles, TrendingUp, Wallet } from "lucide-react"
    ...
    <div className="flex flex-col min-h-screen">
        ...
        <div className="container mx-auto text-center max-w-5xl relative z-10">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-bold tracking-tighter mb-6 bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70">
              AI-Powered Personal <br className="hidden md:block" /> Finance Tracker
            </h1>
            
            <p className="text-lg md:text-xl text-muted-foreground mb-10 max-w-2xl mx-auto leading-relaxed">
              Track expenses, control your budget, and let intelligent AI manage your money. 
              Built for speed, styled for the future.
            </p>

            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              <Link href="/signup" className="w-full sm:w-auto">
                <Button size="lg" className="w-full sm:w-auto h-12 px-8 text-base bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all active:scale-95">
                  Get Started <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
              <Link href="/login" className="w-full sm:w-auto">
                <Button size="lg" variant="outline" className="w-full sm:w-auto h-12 px-8 text-base border-border/50 bg-background/50 hover:bg-muted/50 backdrop-blur-sm transition-all active:scale-95">
                  Live Demo
                </Button>
              </Link>
            </div>
              ...
              <div className="aspect-[16/9] w-full bg-gradient-to-br from-muted/20 via-background to-muted/10 p-8 flex items-center justify-center relative overflow-hidden">
                 ...
                 <div className="absolute bottom-10 right-10 w-80 h-48 rounded-xl border border-border/50 bg-background/40 backdrop-blur-md shadow-sm p-4 flex items-end justify-between gap-2">
                    {[45, 72, 38, 91, 55, 67, 80].map((height, i) => (
                      <div key={i} className="flex-1 bg-primary/30 rounded-t-sm" style={{ height: `${height}%` }} />
                    ))}
                 </div>
                 {/* Chat Mockup */}
                 <div className="absolute bottom-10 left-10 w-72 h-40 rounded-xl border border-border/50 bg-background/60 backdrop-blur-md shadow-sm p-4 flex flex-col justify-end space-y-3">
                    <div className="self-end bg-primary/20 p-2 rounded-lg text-xs w-3/4">Log my 15 euro lunch</div>
                    <div className="self-start bg-muted/50 p-2 rounded-lg text-xs w-4/5 flex items-center gap-2">
                      <Bot size={12} className="text-primary"/> Logged! You have €120 remaining this week.
                    </div>
                 </div>
              </div>
            </div>
      ...
      <section className="py-24 bg-muted/5 border-y border-border/50 relative">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4">Everything you need to master your money</h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Purpose-built tools designed to give you clarity and control over your financial life.</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: "AI Expense Assistant", desc: "Automatically add expenses through natural chat.", icon: Bot },
              { title: "Smart Budget Tracking", desc: "Monthly budgets with automatic weekly limits.", icon: Wallet },
              { title: "Spending Insights", desc: "Visual analytics and trend charts.", icon: PieChart },
              { title: "Secure Data", desc: "Powered by Supabase enterprise authentication.", icon: ShieldCheck }
            ].map((feat, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
      ...
      <section className="py-24 relative overflow-hidden">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Three simple steps to financial clarity.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            {[
              { step: "01", title: "Set your budget", desc: "Establish your monthly allowance and let us calculate your optimal weekly limits." },
              { step: "02", title: "Track daily", desc: "Log transactions manually or simply tell the AI what you bought." },
              { step: "03", title: "Gain insights", desc: "Chat with the AI for immediate feedback on spending trends and safety margins." }
            ].map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
      ...
      <section className="py-24 bg-muted/10 border-y border-border/50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl flex flex-col lg:flex-row items-center gap-16">
          <div className="flex-1 space-y-6">
            ...
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight">Your personal <br /> financial analyst.</h2>
            <p className="text-lg text-muted-foreground leading-relaxed">
              Don't just view charts. Talk to your data. Our OpenAI integrated assistant can dynamically log transactions, warn you if you're overspending, and summarize your remaining limits instantaneously.
            </p>
          </div>
              ...
              <CardContent className="p-6 space-y-6">
                 ...
                 >
                    ...
                    <div className="bg-muted/50 border border-border/50 px-4 py-3 rounded-2xl rounded-tl-sm text-sm shadow-sm max-w-[85%]">
                      <span className="italic text-muted-foreground text-xs block mb-1">Executing Tool: addExpense</span>
                      Expense added successfully. You have €3,200 remaining this week.
                    </div>
                 </motion.div>
              </CardContent>
            </Card>
      ...
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center max-w-3xl relative z-10">
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-6">Take control of your finances today.</h2>
          <p className="text-xl text-muted-foreground mb-10">Join the next generation of expense tracking. Free while in beta.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/signup">
              <Button size="lg" className="h-12 px-8 text-base bg-foreground text-background hover:bg-foreground/90 transition-all active:scale-95 shadow-xl">
                Create Account
              </Button>
            </Link>
            <Link href="/login">
              <Button size="lg" variant="outline" className="h-12 px-8 text-base border-border/50 bg-background/50 hover:bg-muted/50 backdrop-blur-sm transition-all active:scale-95">
                Start Tracking
              </Button>
            </Link>
          </div>
