...
export async function POST(req: Request) {
  ...
  try {
    ...
    let { messages } = body as { messages: any[] };
...
- When a user asks about their current spending, budget, remaining money, or weekly limit, use getBudgetSummary (and optionally getExpenses for more detail).
...
- After executing any tool, ALWAYS respond with a clear, human-friendly message describing what happened. Never just say that a tool was called.
- Format currency with € symbols.
- For "show my current spending" or similar, calculate totals, highlight the biggest categories, and explain their situation in simple language.
- If a user is close to their weekly limit, warn them politely and suggest how much they can safely spend.
...
- "add expense of 10 for dinner under food category" → addExpense(amount: 10, category: "Food", description: "dinner")
`;
    ...
    // Convert UI messages from the client into model messages for streamText
